...
function getData(source) {
    ...
    var $result=$("<table>");
    var $title=$("<tr style='border-bottom: 2px solid #ddd;height:30px;'>");
    ...
    $.getJSON(communitySite+"/"+source+".json",function(data){
        if(data){
            ...
            for (var index=0; index < listLength; index++){
                ...
                var $line = $("<tr onclick=window.open('"+communitySite+"/t/"+topics[index].slug+"/"+topics[index].id+"') style='cursor: pointer;'>");
                $result.append($line);
